...
// Utility function to handle safe division and avoid NaN
            ...
            if (course.Marks.Mark && course.Marks.Mark.Assignments.Assignment) {
                ...
                for (let assignment of [...course.Marks.Mark.Assignments.Assignment].reverse()) {
                    ...
                    if (assignment.Points.includes(' / ') || assignment.edited) {
                        ...
                        if (
                            ...
                            assignment.scorePercent = safeDivide(assignment.points, assignment.total) * 100
                            ...
                            for (let type of Object.values(course.scoreTypes)) {
                                ...
                                if (type.total > 0) {
                                    scoreSum += safeDivide(type.score, type.total) * type.weight
                            ...
                            if (totalSum > 0) {
                                let color = getColor(safeDivide(scoreSum, totalSum) * 100)
                                let grade = safeDivide(scoreSum, totalSum) * (course.fourPoint ? 4 : 100)
                ...
                for (let type of Object.values(course.scoreTypes)) {
                    if (totalWeight > 0) {
                        ...
                        let percent = type.total ? safeDivide(type.score, type.total) : 0
